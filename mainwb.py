    ...
    while update_iter < args.total_timesteps:
        ...
        for tidx in train_indices:
            ...
            if ((episode_num % args.log_interval == 0 or episode_num % len(train_tasks)/2 == 0) or episode_num == 1 ):
                ...
                wandb.log({'fps':fps,
                    "critic_loss":float(alg_stats['critic_loss']), "actor_loss":float(alg_stats['actor_loss']),
                    "episode_reward":float(data['episode_reward']), "eprewmean":float(safemean([epinfo['r'] for epinfo in epinfobuf])),
                    'eplenmean':float(safemean([epinfo['l'] for epinfo in epinfobuf])), "episode_num":episode_num,
                    "sampling_loop":sampling_loop, "buffer_size":replay_buffer.size_rb(), "adjusted_no_steps":adjusted_no_steps},
                    step=update_iter)
                ...
                if 'actor_mmd_loss' in alg_stats:
                    ...
                    wandb.log({"critic_mmd_loss":float(alg_stats['critic_mmd_loss']),
                        "actor_mmd_loss":float(alg_stats['actor_mmd_loss'])}, step=update_iter)

                if 'beta_score' in alg_stats:
                     logger.record_tabular("beta_score", float(alg_stats['beta_score']))
                     wandb.log({"beta_score":float(alg_stats['beta_score'])}, step=update_iter)

                logger.dump_tabular()
                print(("Total T: %d Episode Num: %d Episode Len: %d Reward: %f") %
                      (update_iter, episode_num, data['episode_timesteps'], data['episode_reward']))
                wandb.log({"Total T": update_iter, "Episode Num": episode_num, "Episode Len": data['episode_timesteps'],
                      "Reward": data['episode_reward']}, step=update_iter)
            ...
            if timesteps_since_eval >= args.eval_freq:
                ...
                wrt_csv_eval.write({'nupdates':update_iter,
                                   ...
                                   'episode_num':episode_num,
                                   'sampling_loop':sampling_loop})
                wandb.log({'eval_eprewmean':eval_temp, 'train_eprewmean':train_subset_tasks_eval}, step=update_iter)
